...
{
    static RealCoordinates start;
    static RealCoordinates end;

    int used;
    int to_use;
    int total;
    Pathfinding[] neighbors = new Pathfinding[4]; // 0: nord, 1: est, 2: sud, 3: ouest
    boolean open;
    
    /*
    public Pathfinding(int i, int j, int used, int di, int dj)
    {
    ...
    static int calculateDistance(int di, int dj)
    {
        int x = Math.abs(di-end.round().x());
        int y = Math.abs(dj-end.round().y());
        return x + y;
    }
